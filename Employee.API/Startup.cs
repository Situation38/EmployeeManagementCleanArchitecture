...
namespace Employee.API
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            //Nous allons maintenant plonger dans l'implémentation de l'API. Dans le projet API,
            //nous ajouterons des dépendances comme Automapper, swagger, MediatR dans le fichier startup.cs
            services.AddControllers();
            services.AddDbContext<EmployeeContext>(m => m.UseSqlServer(Configuration.GetConnectionString("EmployeeDB")), ServiceLifetime.Singleton);
            services.AddSwaggerGen(c => {
                c.SwaggerDoc("v1", new OpenApiInfo
